    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            ...
            if isinstance(field.widget, forms.CheckboxInput):
                ...
                pass # No default class for RadioSelect, specific styling handled in template if needed
            elif isinstance(field.widget, forms.Select):
                new_classes.append('form-select')
            elif isinstance(field.widget, forms.ClearableFileInput):
                new_classes.append('form-control') # Or 'form-control-file' depending on Bootstrap version
...
class RequestCreateForm(BootstrapFormMixin, forms.ModelForm):
    ...
    related_equipment = forms.ModelChoiceField(
        ...
        queryset=Equipment.objects.select_related('status').filter(status__is_decommissioned=False).order_by('name'),
        required=False,
    ...
    class Meta:
        ...
        fields = [
            ...
            'related_equipment', 'department_location', 'desired_completion_date'
        ]
        labels = {
            'subject': _("თემა"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'priority' in self.fields:
            ...
            if current_choices and (not current_choices[0][0] == ''):
...
class RequestStaffUpdateForm(BootstrapFormMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'status', 'assigned_to', 'priority', 'resolution_notes', 'update_notes'
        ]
        labels = {
            'status': _("სტატუსი"),
    ...
    def save(self, commit=True):
        ...
        terminal_statuses_for_resolution = ['completed', 'rejected', 'closed']
        if 'status' in self.changed_data:
            if instance.status in terminal_statuses_for_resolution and not instance.resolved_at:
                instance.resolved_at = timezone.now()
            elif instance.status not in terminal_statuses_for_resolution and instance.resolved_at:
        ...
        if commit:
            ...
            if (status_changed or notes_provided) and self.acting_user:
                ...
                if status_changed:
                    status_change_text = _("Status changed from '{old}' to '{new}'.").format(
                    ...
                    if not notes_provided:
                        log_notes = status_change_text
                    else:
                         log_notes = f"{status_change_text}\n{_('Notes')}: {update_notes_text}"
...
class RequestUpdateLogForm(BootstrapFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        status_choices_with_empty = [('', _('---------'))] + Request.STATUS_CHOICES
        if 'old_status' in self.fields:
...
class UserRequestCommentForm(BootstrapFormMixin, forms.Form):
    comment_text = forms.CharField(
        ...
        widget=forms.Textarea(attrs={'rows': 3, 'placeholder': _("დაამატეთ თქვენი კომენტარი ან დამატებითი ინფორმაცია...")}),
        required=True
    )

    # We don't need an __init__ that pops 'user' if the form itself doesn't use it directly.
    # The view will handle associating the comment with the user.