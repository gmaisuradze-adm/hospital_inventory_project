...
router.get('/my-dashboard', authenticate, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'User not authenticated' });
...
router.get('/charts/requests-by-status', authenticate, async (req: Request, res: Response) => {
  try {
...
router.get('/charts/inventory-trends', authenticate, async (req: Request, res: Response) => {
  try {
